...
export async function getGameStats(originalPlayerId: number): Promise<GameStatsResult> {
    const [agg] = await replayModel.aggregate([
        ...
        { $group: {
            ...
            pBlockCount: { $sum: '$stats.player.attacksBlocked' },
            pBlockDmg: { $sum: '$stats.player.damageBlocked' },
            ...
            eBlockCount: { $sum: '$stats.enemy.attacksBlocked' },
            eBlockDmg: { $sum: '$stats.enemy.damageBlocked' },
    ...
    return {
        ...
        stats: {
            player: {
                ...
                attacksBlocked: agg.pBlockCount ?? 0,
                damageBlocked: agg.pBlockDmg ?? 0,
            ...
            enemy: {
                ...
                attacksBlocked: agg.eBlockCount ?? 0,
                damageBlocked: agg.eBlockDmg ?? 0,
