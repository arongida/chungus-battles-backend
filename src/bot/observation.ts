...
export function buildItemView(item: Item): ItemView {
    return {
        ...
        triggerTypes: toStringArray(item.triggerTypes),
        ...
        skillAffectedStats: statBlockFromAffectedStats(item.skillAffectedStats),
        skillAffectedEnemyStats: statBlockFromAffectedStats(item.skillAffectedEnemyStats),
        skillId2: item.skillId2,
        skillName2: item.skillName2,
        skillAffectedStats2: statBlockFromAffectedStats(item.skillAffectedStats2),
        skillAffectedEnemyStats2: statBlockFromAffectedStats(item.skillAffectedEnemyStats2),
        sold: item.sold,
...
export function buildTalentView(talent: Talent): TalentView {
    return {
        ...
        activationRate: talent.activationRate ?? 0,
        base: talent.base ?? 0,
        scaling: talent.scaling ?? 0,
        totalActivations: talent.totalActivations ?? 0,
        totalDamageDealt: talent.totalDamageDealt ?? 0,
        totalHealingDone: talent.totalHealingDone ?? 0,
        totalGoldGained: talent.totalGoldGained ?? 0,
        totalXpGained: talent.totalXpGained ?? 0,
        totalHealingPrevented: talent.totalHealingPrevented ?? 0,
    };
}
...
export function buildPlayerView(player: Player): PlayerView {
    return {
        ...
        shopLocked: player.lockedShop.length > 0,
