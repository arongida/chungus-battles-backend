...
export function rollArchetype(seed: number): ArchetypeWeights {
    ...
    return ARCHETYPES[ARCHETYPE_IDS[Math.floor(rand() * ARCHETYPE_IDS.length)]];
}
...
export function tagAffinity(a: ArchetypeWeights, tag: string): number {
    return (a.classAffinity as Record<string, number>)[tag] ?? a.subclassAffinity[tag] ?? 1;
}
